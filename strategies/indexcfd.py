...
def score_ticker(ticker: str, cfg: dict, gates: dict) -> dict:
    """Run 6 scored checks + apply 2 gates for an Index CFD.

    Uses the same 6 checks as ITA/US but applied to index ETF proxies.
    Leverage is 20:1 (ESMA major indices) instead of 5:1 (stocks).
    ...
    Scored checks (6):
      ...
      5. MFI > threshold        (money flow via ETF proxy volume)
    ...
    index_labels = cfg.get("index_labels", {})
